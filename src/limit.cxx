...
int main(int argc, char * argv[]) {
  ...
  double val, err;
  int keep_going;
  if (finalParams[4] < poisson_threshold) keep_going = hcubature(1, f_poisson, &finalParams, 4, xmin, xmax, 0, 0, 1e-3, ERROR_INDIVIDUAL, &val, &err);
  else keep_going = hcubature(1, f_gaussian, &finalParams, 4, xmin, xmax, 0, 0, 1e-3, ERROR_INDIVIDUAL, &val, &err);
